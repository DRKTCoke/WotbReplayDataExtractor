...
潜在场均是后续 rating 设计前需要先补齐的独立字段，不覆盖现有「均伤 / 场均伤害」。当前已把计算规则固化在 `PotentialDamage`，但还未接入 Web 展示，因为缺少逐目标事件解析。
...
- 还未把 `potentialDamageAvg` 接入前端或 rating，因为逐目标事件字段尚未验证。

注意：进入 rating 公式设计前，需要先确认潜在场均字段和权重方案，不要直接把它并入现有 `RatingAnalyzer`。
