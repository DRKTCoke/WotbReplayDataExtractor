...
├── build.bat                  # Python PyInstaller 构建
├── Data/                      # 示例回放
├── dist/                      # Python PyInstaller 产物
└── java/
    ├── README.md
    ├── pom.xml                # 父 POM：Spring Boot 4.1.0, Java 21
    ├── settings.xml           # 本地 Maven 配置，独立仓库 .m2repo
    ├── settings-docker.xml    # Docker Maven 配置
    ├── docker-compose.yml
    ├── build-desktop.bat      # jpackage 离线 exe 构建脚本
    ├── dist-desktop/          # jpackage 产物
    ├── wotb-core/             # Java 核心库
    ...
    │   │       ├── Battle.java
    │   │       └── PlayerResult.java
    │   └── src/test/java/com/wotb/core/
    │       └── ParityTest.java      # Java 回归测试
    ├── wotb-web/              # Spring Boot API + 桌面模式入口
    ...
    │   └── src/test/java/com/wotb/web/
    │       └── WebApiTest.java
    └── frontend/              # Vue 3 前端
        ├── src/
        │   ├── main.js
        │   └── App.vue                 # 单文件组件（310 行，无 router）
        ├── index.html
        ├── package.json
        ├── vite.config.js
        └── Dockerfile                  # nginx 部署（Web 版）
```
...
1. Java `ReplayParser`、`PlayerResult`、`Columns`、`ExcelExporter`。
2. Java Web API `Mapper` 和前端预览列。
...
3. 同步到 Java resources：`copy tankopedia.json java\wotb-core\src\main\resources\tankopedia.json`。
...
- `Columns.java` 是 Java 侧列定义的数据源，前端和导出都依赖它；新增字段先加 `Col` 记录。
