...
└── java/
    ...
    ├── frontend/             # 【共享】Vue 3 前端（双 HTML 入口，无 router）
    │   ├── src/{main.js, App.vue, extended.js, ExtendedApp.vue}
    │   ├── index.html  extended.html  package.json  vite.config.js  .npmrc
    ├── offline/              # 离线版打包
...
- 双 HTML 入口，无 Vue Router、无组件库：`index.html` 挂载 `App.vue`，`extended.html` 挂载 `ExtendedApp.vue`。
...
- `/api/preview` 的每个 `BattleDto` 会返回 `battleRawColumns` / `battleRaw`（battle_results 顶层 protobuf 字段）和 `rawColumns`；每个 `PlayerRow` 会返回 `raw`（玩家战绩 protobuf 字段）。字段名保持 `#字段号`，用于在 Web 端展示当前已解出的原始字段。
- 前端分两个入口：`index.html` / `App.vue` 保持原参数解析页面，不展示扩展字段、raw 字段、trace 或 rating 入口；`extended.html` / `ExtendedApp.vue` 专门展示扩展字段、`data.wotreplay` 诊断、raw 字段和实时 rating；部署访问推荐路径为 `/extended`，实际静态文件为 `/extended.html`。
- 主要交互：
...
- 原解析页：`App.vue` 的 `PLAYER_LABELS` / `AGG_LABELS` 只覆盖旧页面字段；扩展分析页：`ExtendedApp.vue` 维护扩展字段与 rating 的显示名。
- 导出层：`Columns.java`（单场 xlsx 表头）、`ExcelExporter` 的汇总列、Python `STAT_COLUMNS` / `export_aggregate_xlsx`。

> 这是有意的取舍：API 干净、可多语言，但中文标签存在多份（前端 + 导出）。**改任一列名，务必同步对应前端入口的映射与三处导出标签。**
>
...
Web 版额外提供实时 rating 接口：`POST /api/rating`。它只基于本次 multipart 上传的回放计算，不落库、不读取历史记录。返回列定义来自 `Mapper.ratingColumns()`，行数据来自 `RatingAnalyzer.compute(...)`。

当前指标：

- `rating`：复用上面的相对评分，按上传批次计算后取选手场均。
- `kast`：本批次中满足 Kill / Assist damage / Survive 的场次占比。当前未解析完整战斗事件流，因此无法判断 trade，KAST 的 T 暂不计入。
- `contribution`：选手有效贡献 EC 在其所在队伍 EC 中的占比。
- `influence`：影响力指数，`100` 约等于 7 人队内平均影响；由 EC 队内份额、击杀队内份额、击伤敌人队内份额加权归一化。
- `damage_avg`：均伤。
- `kills` / `kills_avg`：总人头 / 场均人头。

### 潜在场均

潜在场均是后续 rating 设计前需要先补齐的独立字段，不覆盖现有「均伤 / 场均伤害」。当前已把计算规则固化在 `PotentialDamage`，并接入 `/api/preview` 的扩展玩家字段、`/api/rating` 的 `potential_damage_avg` / `potential_damage_supplement_avg`，但逐目标事件解析仍未完成。

定义：

- 对每名玩家，每场先保留实际伤害 `actualDamage`。
- 只检查该玩家击杀的目标。
- 对每个被击杀目标，需要知道：攻击者对该目标造成的伤害、对该目标的击穿次数、攻击者当前车辆炮伤 `alphaDamage`。
- 若 `目标伤害 / 击穿次数 < 0.9 × alphaDamage`，则该目标伤害补增至 `ceil(击穿次数 × 0.9 × alphaDamage)`。
- 本场 `potentialDamage = actualDamage + 所有击杀目标补增值`。
- `potentialDamageAvg = sum(potentialDamage) / battles`。
- 未解析到逐击杀目标明细时，本场 `potentialDamage` 暂等于实际伤害、补增为 `0`，并用 `potential_damage_detail=未解析` 标记来源。

尚需解析的数据：

- `data.wotreplay` 中逐炮 / 逐目标事件：攻击者、受害者、伤害、是否击穿、击杀归属。
- 车辆炮伤 `alphaDamage`：当前已从 `tankopedia.json` 读取默认炮伤；若后续 `data.wotreplay` 能解析出实际炮模块/弹种，应优先使用事件中的实际弹种伤害。

当前进度：

- `DataReplayParser` 已能解析 `data.wotreplay` 的顶层包摘要：客户端版本、packet type 统计、packet type 最大 payload、type=8 entity method subtype 统计、subtype 最大 payload、时间范围、最大 payload 长度。
- `/api/preview` 的单场 `BattleDto.replayTrace` 会返回该摘要，并额外返回 `packetGroups` / `entityMethodGroups` 诊断行：count、payload min/avg/max、clock 范围、首个 payload hex 前缀，以及 payload 中疑似出现本场账号 ID / 车辆 ID 的次数和样本，便于用真实回放样本反推伤害/击穿/击杀事件所在 subtype。
- 账号/车辆 ID 命中是 little-endian 字节模式的启发式诊断，只用于缩小逆向范围；不能直接当作已确认的伤害事件字段。
- `PotentialDamage.apply(...)` 已在 Web 预览、实时 rating、Excel 导出前调用；新扩展页会显示单场 `potential_damage` / `potential_damage_supplement` / `potential_damage_detail`，rating 表显示 `potential_damage_avg` / `potential_damage_supplement_avg`。
- 当前尚未从 `data.wotreplay` 验证逐目标伤害/击穿/击杀事件字段，因此补增仍为 0；下一步应继续用 `replayTrace` 诊断定位可解析 subtype。

注意：进入 rating 公式设计前，需要先确认潜在场均字段和权重方案，不要直接把它并入现有 `RatingAnalyzer`。当前接入只展示潜在均伤，不改变 rating 分数公式。
...
- 列的 `key`（snake_case）在 API、前端映射、导出三方一致；中文显示名在对应前端入口映射 + 三处导出标签一致。

如修改字段解释或列名，必须同步：

1. Java `ReplayParser`、`PlayerResult`、`Columns`、`ExcelExporter`（含汇总列）。
2. Java Web API `Mapper`（`/api/columns` 只回英文 key）+ 对应前端入口的显示名映射（原页 `App.vue`，扩展页 `ExtendedApp.vue`）。
...
车辆库是**单一来源** `common/tankopedia.json`，两侧共用，无需手动同步。除车名、等级、类型、国家外，当前还保存 `alphaDamage`：从 BlitzKit `tanks.pb` 的最高等级炮模块读取第一种炮弹伤害，作为默认炮伤。
...
- 如果只改 Web UI 的显示名，改对应前端入口的标签映射即可；API 只回英文 key，不要把中文加回 API。
- `Columns.java` 是 Java 侧列的 key/顺序/取值与**单场 xlsx 表头**的来源；新增字段先加 `Col` 记录，再同步扩展页映射、需要展示该字段的汇总列与 Python；不要让扩展字段自动进入原解析页。
